...
namespace IvyFEM
{
    public class EMWaveguide3DPMLTDFEM : FEM
    {
        //---------------------------------
        // ※ポートの設定順序
        // ABC境界、参照(観測)面、励振源の順
        ...
        /// 実際に計算するステップ数
        /// </summary>
        public int ExecTimeLoopCnt { get; set; } = 0;
        /// <summary>
        /// 計算時刻インデックス
        /// </summary>
        ...
        */
        ...
        //-------------------------------
        // formulation (2)
        private double[][] PEPrev2s = null;
        private double[][] PEPrev3s = null;
        //-------------------------------
        ...
        public EMWaveguide3DPMLTDFEM(FEWorld world)
        {
            World = world;
        }
        ...
        public override void Solve()
        {
            ...
            CalcB();
            */
            if (ExecTimeLoopCnt != 0 && TimeIndex >= ExecTimeLoopCnt)
            {
                // スキップ
                B = null;
            }
        ...
        private void CalcA()
        {
            ...
            */
            ...
            //-------------------------------
            //-------------------------------
            PEPrev2s = new double[feCnt][];
            PEPrev3s = new double[feCnt][];
            //-------------------------------
        ...
        private void _CalcB()
        {
            ...
            foreach (uint feId in feIds)
            {
                ...
                */
                ...
                //-------------------------------
                //-------------------------------
                pEPrev2 = PEPrev2s[feId - 1];
                if (pEPrev2 == null)
                {
                    pEPrev2 = new double[elemEdgeNodeCnt];
                    PEPrev2s[feId - 1] = pEPrev2;
                }
                pEPrev3 = PEPrev3s[feId - 1];
                if (pEPrev3 == null)
                {
                    pEPrev3 = new double[elemEdgeNodeCnt];
                    PEPrev3s[feId - 1] = pEPrev3;
                }
                pEPrev2.CopyTo(pEPrev3, 0);
                pE.CopyTo(pEPrev2, 0);
                //-------------------------------
                        ...
                        for (int col = 0; col < elemEdgeNodeCnt; col++)
                        {
                            ...
                            if (maPML != null)
                            {
                                ...
                                */
                                ...
                                //----------------------------------
                                // formulation (2)
                                bValue += (1.0 / (dt * dt)) * (pE[col] - 2.0 * pEPrev2[col] + pEPrev3[col])
                                    * mValueX;
                                //----------------------------------
                            }
                            B[rowEdgeNodeId] +=
