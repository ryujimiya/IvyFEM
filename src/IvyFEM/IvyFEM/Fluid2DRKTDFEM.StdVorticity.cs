...
namespace IvyFEM
{
    ...
    {
        private void CalcStdGVorticity1stEquationKMF(
                ...
                for (int ipPt = 0; ipPt < ip.PointCount; ipPt++)
                {
                    ...
                    for (int row = 0; row < wElemNodeCnt; row++)
                    {
                        ...
                        for (int col = 0; col < wElemNodeCnt; col++)
                        {
                            ...
                            K[rowNodeId, colNodeId] += kww1 + kww2;
                            M[rowNodeId, colNodeId] += m;
                        }
                    }
        ...
        private void CalcStdGVorticity2ndEquationAB(IvyFEM.Linear.DoubleSparseMatrix A, double[] B)
        {
            uint wQuantityId = 0;
            uint pQuantityId = 1;
            int wNodeCnt = (int)World.GetNodeCount(wQuantityId);
