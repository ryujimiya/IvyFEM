...
namespace IvyFEM
{
    ...
    {
        private void CalcSUPGPressurePoissonWithBell1stEquationKMF(
            ...
            foreach (uint feId in feIds)
            {
                ...
                for (int ipPt = 0; ipPt < ip.PointCount; ipPt++)
                {
                    ...
                    for (int iNode = 0; iNode < vElemNodeCnt; iNode++)
                    {
                        ...
                        double[] prevU = prevFV.GetDoubleValue(coId, FieldDerivativeType.Value);
                        double[] vel = new double[vDof];
                        for (int iDof = 0; iDof < vDof; iDof++)
                        {
                            double vValue = U[nodeId * vDof + iDof];
                            vel[iDof] = (3.0 * vValue - 4.0 * u[iDof] + prevU[iDof]) / (2.0 * dt);
                        }
                    ...
                    for (int row = 0; row < vElemNodeCnt; row++)
                    {
                        ...
                        for (uint colQuantityId = pQuantityId; colQuantityId  < quantityCnt; colQuantityId++)
                        {
                            ...
                            for (int col = 0; col < colpElemNodeCnt; col++)
                            {
                                ...
                                for (int rowDof = 0; rowDof < vDof; rowDof++)
                                {
                                    double pValue = U[coloffsetp + offsetp + colNodeId];
                    ...
                    double[,,] rmivjdvdt = new double[vDof, vDof, vElemNodeCnt];
                    ...
                    for (int iDof = 0; iDof < vDof; iDof++)
                    {
                        ...
                        for (int jDof = 0; jDof < vDof; jDof++)
                        {
                            for (int jNode = 0; jNode < vElemNodeCnt; jNode++)
                            {
                                ...
                                rmivjdvdt[iDof, jDof, jNode] = 0;
                                if (iDof == jDof)
                                {
                                    rmivjdvdt[iDof, jDof, jNode] +=
                                        rho * vN[jNode]; // dv/dt項
                    ...
                    for (int row = 0; row < vElemNodeCnt; row++)
                    {
                        ...
                        for (uint colQuantityId = pQuantityId; colQuantityId < quantityCnt; colQuantityId++)
                        {
                            ...
                            for (int col = 0; col < colpElemNodeCnt; col++)
                            {
                                ...
                                for (int rowDof = 0; rowDof < vDof; rowDof++)
                                {
                                    double pValue = U[coloffsetp + colNodeId];
                                    // for Navier-Stokes
                                    F[rowNodeId * vDof + rowDof] +=
                                        -kvp[rowDof, 0] * pValue;
                                }
                ...
                for (int row = 0; row < vElemNodeCnt; row++)
                {
                    ...
                    for (int rowDof = 0; rowDof < vDof; rowDof++)
                    {
                        F[rowNodeId * vDof + rowDof] += rho * g[rowDof] * vSN[row];
                    }
                }
            }
        }

        private void CalcSUPGPressurePoissonWithBell2ndEquationAB(IvyFEM.Linear.DoubleSparseMatrix A, double[] B)
        {
            int quantityCnt = World.GetQuantityCount();
            System.Diagnostics.Debug.Assert(quantityCnt == 7);
            uint vQuantityId = 0;
                    ...
                    for (uint rowQuantityId = pQuantityId; rowQuantityId < quantityCnt; rowQuantityId++)
                    {
                        ...
                        for (int row = 0; row < rowpElemNodeCnt; row++)
                        {
                            ...
                            for (int col = 0; col < vElemNodeCnt; col++)
                            {
                                ...
                                double[,] kpv = new double[pDof, vDof];
                                double[,] kpvdvdt = new double[pDof, vDof];
                                for (int colDof = 0; colDof < vDof; colDof++)
                                {
                                    kpv[0, colDof] =
                                    ...
                                    kpvdvdt[0, colDof] =
                                        detJWeight * (1.0 / rho) * taum *
                                        (rowpNu[0][row] * rmivjdvdt[0, colDof, col] +
                                        rowpNu[1][row] * rmivjdvdt[1, colDof, col]);
                                }
