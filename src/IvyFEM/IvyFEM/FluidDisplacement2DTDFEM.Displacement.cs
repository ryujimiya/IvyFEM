...
namespace IvyFEM
{
    public partial class FluidDisplacement2DTDFEM
    {
        private void InitDisplacement()
        {
            ...
            //int cNodeCnt = (int)World.GetNodeCount(dQuantityId);
        ...
        private void CalcABDisplacement(IvyFEM.Linear.DoubleSparseMatrix A, double[] B)
        {
            int nDim = 2;
            uint vQuantityId = 0;
            uint pQuantityId = 1;
                ...
                for (int ipPt = 0; ipPt < ip.PointCount; ipPt++)
                {
                    ...
                    for (int iNode = 0; iNode < vElemNodeCnt; iNode++)
                    {
                        ...
                        for (int iDof = 0; iDof < vDof; iDof++)
                        {
                            ...
                            vel[iDof] = (gamma / (beta * dt)) * (vValue - prevU[iDof]) +
                                (1.0 - gamma / beta) * prevVel[iDof] +
                                dt * (1.0 - gamma / (2.0 * beta)) * prevAcc[iDof];
                        }
                    ...
                    for (int row = 0; row < vElemNodeCnt; row++)
                    {
                        ...
                        for (int col = 0; col < pElemNodeCnt; col++)
                        {
                            ...
                            for (int rowDof = 0; rowDof < vDof; rowDof++)
                            {
                                A[rowNodeId * vDof + rowDof, pOffset + colNodeId] += kvp[rowDof, 0];
                                A[pOffset + colNodeId, rowNodeId * vDof + rowDof] += -kvp[rowDof, 0];
                            }
                        }
                    }
                    ...
                    for (int row = 0; row < pElemNodeCnt; row++)
                    {
                        ...
                        for (int col = 0; col < vElemNodeCnt; col++)
                        {
                            ...
                            for (int colDof = 0; colDof < vDof; colDof++)
                            {
                                A[pOffset + rowNodeId, colNodeId * vDof + colDof] += kpv[0, colDof];
                            }
                        }
                    }
                    // kpp
                    for (int row = 0; row < pElemNodeCnt; row++)
                    {
                        ...
                        for (int col = 0; col < pElemNodeCnt; col++)
                        {
                            ...
                            A[pOffset + rowNodeId, pOffset + colNodeId] += kpp[0, 0];
                        }
                    }
