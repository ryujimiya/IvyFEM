...
namespace IvyFEM
{
    ...
    {
        private void SetVorticity1stEquationSpecialBC(IvyFEM.Linear.DoubleSparseMatrix A, double[] B)
        {
            SetVorticity1stEquationDirichletBCOfVorticityForTangentialFlow(A, B);
            SetVorticity1stEquationDirichletBCOfVorticityForOutflow(A, B);
        }

        private void SetVorticity2ndEquationSpecialBC(IvyFEM.Linear.DoubleSparseMatrix A, double[] B)
        {
            SetVorticity2ndEquationDirichletBCOfStreamForOutflow(A, B);
        }
        ...
        private void SetVorticity1stEquationDirichletBCOfVorticityForTangentialFlow(
                ...
                foreach (uint feId in feIds)
                {
                    ...
                    for (int row = 0; row < elemNodeCnt; row++)
                    {
                        ...
                        for (int colNodeId = 0; colNodeId < pNodeCnt; colNodeId++)
                        {
                            ...
                            double u2 = U[offset + colNodeId];
                            B[rowNodeId] += -value * u2;
                        }
        ...
        private void SetVorticity1stEquationDirichletBCOfVorticityForOutflow(
            IvyFEM.Linear.DoubleSparseMatrix A, double[] B)
        {
            uint wQuantityId = 0;
            uint pQuantityId = 1;
        ...
        private void SetVorticity2ndEquationDirichletBCOfStreamForOutflow(
            IvyFEM.Linear.DoubleSparseMatrix A, double[] B)
        {
            uint wQuantityId = 0;
            uint pQuantityId = 1;
            ...
            for (int portId = 0; portId < portCnt; portId++)
            {
                ...
                foreach (uint feId in feIds)
                {
                    ...
                    for (int row = 0; row < elemNodeCnt; row++)
                    {
                        ...
                        {
                            double value = 0;
                            double u1 = U[colNodeId];
                            B[rowNodeId] += -value * u1;
                        }
                        for (int colNodeId = 0; colNodeId < pNodeCnt; colNodeId++)
                        {
                            int colCoId = World.Node2Coord(pQuantityId, colNodeId);
