...
namespace IvyFEM
{
    public class MeshUtils2D
    {
        ...
        {
            double area = CadUtils2D.TriArea(p0, p1, p2);
        ...
        public static bool CheckTri(IList<MeshPoint2D> pts, IList<MeshTri2D> tris)
        {
            ...
            {
                if (pts[(int)iPt].Element >= 0)
                {
                    System.Diagnostics.Debug.Assert(pts[(int)iPt].Node >= 0 && pts[(int)iPt].Node < 3);
                    int itri0 = pts[(int)iPt].Element;
                    uint inoel0 = pts[(int)iPt].Node;
                              ...
                              IList<MeshPoint2D> points, IList<MeshTri2D> tris)
        {
            ...
            points[(int)iInsPt].Element = iTri0;
            points[(int)iInsPt].Node = 0;
            points[(int)oldTri.V[0]].Element = iTri1;
            points[(int)oldTri.V[0]].Node = 2;
            points[(int)oldTri.V[1]].Element = iTri2;
            points[(int)oldTri.V[1]].Node = 2;
            points[(int)oldTri.V[2]].Element = iTri0;
            points[(int)oldTri.V[2]].Node = 2;
            ...
            IList<MeshPoint2D> points, IList<MeshTri2D> tris)
        {
            ...
            points[(int)iInsPt].Element = (int)itri0;
            points[(int)iInsPt].Node = 0;
            points[(int)old0.V[ino20]].Element = (int)itri0;
            points[(int)old0.V[ino20]].Node = 1;
            points[(int)old0.V[ino00]].Element = (int)itri1;
            points[(int)old0.V[ino00]].Node = 1;
            points[(int)old1.V[ino21]].Element = (int)itri2;
            points[(int)old1.V[ino21]].Node = 1;
            points[(int)old1.V[ino01]].Element = (int)itri3;
            points[(int)old1.V[ino01]].Node = 1;
        ...
        public static bool DelaunayAroundPoint(uint iPt0, IList<MeshPoint2D> points, IList<MeshTri2D> tris)
        {
            ...
            if (points[(int)iPt0].Element == -1)
            {
                return true;
            }

            System.Diagnostics.Debug.Assert(points[(int)iPt0].Element >= 0 &&
                points[(int)iPt0].Element < tris.Count );
            System.Diagnostics.Debug.Assert(tris[points[(int)iPt0].Element].V[points[(int)iPt0].Node] == iPt0);

            uint iTri0 = (uint)points[(int)iPt0].Element;
            uint iTriNo0 = points[(int)iPt0].Node;

            uint iCurTri = iTri0;
            uint iCurTriNo = points[(int)iPt0].Node;
            bool isWall = false;
            while (true)
            {
                ...
                if (tris[(int)iCurTri].G2[iCurTriNo] == -2)
                {
                        ...
                        points[(int)iDiaPt].Point) == 0)
                    {
                        ...
                        // 辺を切り替える
                        // FlipEdgeによってiCurTriは時計回り側の三角形に切り替わる
        ...
        public static bool FlipEdge(uint iTri0, uint iEd0, IList<MeshPoint2D> points, IList<MeshTri2D> tris)
        {
            ...
            points[(int)old0.V[no10]].Element = (int)iTri0;
            points[(int)old0.V[no10]].Node = 0;
            points[(int)old0.V[no00]].Element = (int)iTri0;
            points[(int)old0.V[no00]].Node = 2;
            points[(int)old1.V[no11]].Element = (int)iTri1;
            points[(int)old1.V[no11]].Node = 0;
            points[(int)old1.V[no01]].Element = (int)iTri1;
            points[(int)old1.V[no01]].Node = 2;
            ...
            IList<MeshPoint2D> points, IList<MeshTri2D> tris)
        {
            ...
            uint iIniTri = (uint)points[(int)iPt0].Element;
            uint iIniTriNo = points[(int)iPt0].Node;
        ...
        public static void LaplacianSmoothing(IList<MeshPoint2D> points, IList<MeshTri2D> tris, IList<uint> isntMoves)
        {
            ...
            {  
                ...
                uint iTriIni = (uint)points[(int)iPt].Element;
                uint iNoElCIni = points[(int)iPt].Node;
