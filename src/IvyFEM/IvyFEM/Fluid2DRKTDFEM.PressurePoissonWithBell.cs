...
namespace IvyFEM
{
    ...
    {
        private void SetPressurePoissonWithBell1stEquationSpecialBC(IvyFEM.Linear.DoubleSparseMatrix A, double[] B)
        {
            // Nothing
        }

        private void SetPressurePoissonWithBell2ndEquationSpecialBC(IvyFEM.Linear.DoubleSparseMatrix A, double[] B)
        {
            // for fixed velocity
            SetPressurePoissonWithBell2ndEquationNoConstraintNeumannBCOfPressure(A, B);
            SetPressurePoissonWithBell2ndEquationNormalInflowNeumannBCOfPressure(A, B);
            // for outflow
            SetPressurePoissonWithBell2ndEquationOutflowNeumannBCOfPressure(A, B);
        }

        private void SetPressurePoissonWithBell2ndEquationNoConstraintNeumannBCOfPressure(
                ...
                foreach (uint feId in feIds)
                {
...

                    {
                        ...
                        for (int ipPt = 0; ipPt < ip.PointCount; ipPt++)
                        {
                            ...
                            {
                                int rowoffsetp = offsetps[rowQuantityId - pQuantityId] - offsetp;
                                uint rowpTriFENodeCnt = pTriFENodeCnts[rowQuantityId - pQuantityId];
                                int[] rowpTriFENodes = pTriFENodess[rowQuantityId -  pQuantityId];
        ...
        private void SetPressurePoissonWithBell2ndEquationNormalInflowNeumannBCOfPressure(
            IvyFEM.Linear.DoubleSparseMatrix A, double[] B)
        {
            int quantityCnt = World.GetQuantityCount();
            System.Diagnostics.Debug.Assert(quantityCnt == 7);
                        ...
                        for (int ipPt = 0; ipPt < ip.PointCount; ipPt++)
                        {
                            ...
                            for (uint rowQuantityId = pQuantityId; rowQuantityId  < quantityCnt; rowQuantityId++)
                            {
                                ...
                                for (int row = 0; row < rowpTriFENodeCnt; row++)
                                {
                                    ...
                                    {
                                        int coloffsetp = offsetps[colQuantityId - pQuantityId] - offsetp;
                                        uint colpTriFENodeCnt = pTriFENodeCnts[colQuantityId - pQuantityId];
                                        int[] colpTriFENodes = pTriFENodess[colQuantityId - pQuantityId];
        ...
        private void SetPressurePoissonWithBell2ndEquationOutflowNeumannBCOfPressure(
            IvyFEM.Linear.DoubleSparseMatrix A, double[] B)
        {
            int quantityCnt = World.GetQuantityCount();
            System.Diagnostics.Debug.Assert(quantityCnt == 7);
