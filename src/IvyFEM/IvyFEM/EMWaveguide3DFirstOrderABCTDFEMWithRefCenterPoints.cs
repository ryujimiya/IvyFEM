...
namespace IvyFEM
{
    public class EMWaveguide3DFirstOrderABCTDFEMWithRefPoints : FEM
    {
        //---------------------------------
        // ※ポートの設定順序
        // ABC境界、参照(観測)面、励振源の順
        ...
        /// 観測中央点
        public double[][] RefCenterPoints { get; set; } = null;
        ...
        /// 観測点の電界(時間変化リスト)
        /// </summary>
        public IList<IList<double[]>> RefTimeExyzsss { get; private set; } = new List<IList<double[]>>();

        public EMWaveguide3DFirstOrderABCTDFEMWithRefPoints(FEWorld world)
        {
            World = world;
        }
        ...
        public override void Solve()
        {
            ...
            if (TimeIndex == 0)
            {
                ...
                RefTimeExyzsss.Clear();
                for (int i = 0; i < cnt; i++)
                {
                    var refTimeExyzss = new List<double[]>();
                    RefTimeExyzsss.Add(refTimeExyzss);
                }
            }
            ...
            if (RefPortCount > 0)
            {
                double[][] centerPointExyzs;
                CalcRefCenterPointExyz(
                    CoordExyz,
                    RefCenterPoints,
                    out centerPointExyzs);
                ...
                for (int refIndex = 0; refIndex < RefPortCount; refIndex++)
                {
                    ...
                    double[] fValuexyz = centerPointExyzs[refIndex];
                    var refTimeExyzss = RefTimeExyzsss[refIndex];
                    refTimeExyzss.Add(fValuexyz);
                }
            }
            else
            {
        ...
        private void CalcRefCenterPointExyz(
            double[] coordExyz,
            double[][] centerPoints,
            out double[][] centerPointExyzs)
        {
            uint quantityId = 0;
            int dof = 3; // x, y, z
            System.Diagnostics.Debug.Assert(centerPoints.Length == RefPortCount);
            System.Diagnostics.Debug.Assert(centerPoints[0].Length == dof);

            centerPointExyzs = new double[RefPortCount][];
            for (int refIndex = 0; refIndex < RefPortCount; refIndex++)
            {
                double[] centerPoint = centerPoints[refIndex];
                centerPointExyzs[refIndex] = new double[dof];

                var feIds = World.GetTetrahedronFEsWithPointInside(quantityId, centerPoint);
                System.Diagnostics.Debug.Assert(feIds.Count >= 1);
                double[] exyz = new double[dof];
                for (int feIndex = 0; feIndex < feIds.Count;feIndex++)
                {
                    uint feId = feIds[feIndex];
                    TetrahedronFE tetFE = World.GetTetrahedronFE(quantityId, feId);
                    uint elemNodeCnt = tetFE.NodeCount;
                    double[] L = tetFE.Coord2L(centerPoint);
                    // 中心点におけるEx,Ey,Ezを求める
                    double[] exyz1 = new double[dof];
                    double[][] N = tetFE.CalcEdgeN(L);
                    {
                        for (int iNode = 0; iNode < elemNodeCnt; iNode++)
                        {
                            ...
                            for (int idim = 0; idim < dof; idim++)
                            {
                                double nodeExyz = coordExyz[coId * dof + idim];
                                exyz1[idim] += N[iNode][idim] * nodeExyz;
                            }
                        }
                    }
                    for (int dimIndex = 0; dimIndex < dof; dimIndex++)
                    {
                        exyz[dimIndex] += exyz1[dimIndex];
                    }
                }
                for (int dimIndex = 0;dimIndex < dof; dimIndex++)
                {
                    exyz[dimIndex] /= feIds.Count;
                }
                centerPointExyzs[refIndex] = exyz;
            }
        }
            ...
            out IList<System.Numerics.Complex[]> Sss)
        {
            ...
            for (int refIndex = 0; refIndex < RefPortCount; refIndex++)
            {
                int timeCnt = RefTimeExyzsss[refIndex].Count;
                int tmpDim = RefTimeExyzsss[refIndex][0].Length;
                System.Diagnostics.Debug.Assert(tmpDim == dim);
                double[][] datasRef = new double[timeCnt][];
                for (int timeIndex = 0; timeIndex < timeCnt; timeIndex++)
                {
                    double[] dataxyz = new double[dim];
                    for (int dimIndex = 0; dimIndex < dim; dimIndex++)
                    {
                        dataxyz[dimIndex] = RefTimeExyzsss[refIndex][timeIndex][dimIndex];
                    }
                    datasRef[timeIndex] = dataxyz;
                }
            ...
            //!!!!! FIXME: Z方向固定になっている
            //int dimIndex0 = 0; // 暫定：X方向固定
            //int dimIndex0 = 1; // 暫定：Y方向固定
            int dimIndex0 = 2; // 暫定：Z方向固定
            for (int refIndex = 0; refIndex < RefPortCount; refIndex++)
            {
                double[][] datass = datasRefs[refIndex];
                System.Diagnostics.Debug.Assert(datass.Length == dataCnt);
                System.Diagnostics.Debug.Assert(datass[0].Length == dim);
                ...
                {
                    freqDomainDatass[freqIndex] = new System.Numerics.Complex[dim];
                }
                {
                    double[] datas = new double[dataCnt];
                    for (int timeIndex = 0; timeIndex < dataCnt; timeIndex++)
                    {
                        datas[timeIndex] = datass[timeIndex][dimIndex0];
                    }
                    double[] _freqs;
                    ...
                    //!!!! FIXME: 3方向を考慮する場合dimIndex0の最初のみの条件を課す
                    if (refIndex == 0 /*&& dimIndex0 == 0*/)
                    {
                        freqs = _freqs;
                    }
                    for (int freqIndex = 0; freqIndex < dataCnt; freqIndex++)
                    {
                        ...
                        freqDomainDatass[freqIndex][dimIndex0] = data;
                    }
                }
                ...
                for (int freqIndex = 0; freqIndex < dataCnt; freqIndex++)
                {
                    ...
                    if (freq < StartFrequencyForSMatrix || freq > EndFrequencyForSMatrix)
                    {
                        for (int dimIndex = 0; dimIndex < dim; dimIndex++)
                        {
                            freqDomainAmps[freqIndex] = new System.Numerics.Complex[] {0, 0, 0};
                        }
                        continue;
                    }
                    ...
                    // 振幅
                    // 基本モードしか扱えない
                    System.Numerics.Complex[] freqExyz = freqDomainDatass[freqIndex];
                    for (int dimIndex = 0; dimIndex < dim; dimIndex++)
                    {
                        freqDomainAmps[freqIndex] = freqExyz;
                    }
                }
            ...
            for (int refIndex = 0; refIndex < RefPortCount; refIndex++)
            {
                ...
                for (int i = 0; i < dataCnt; i++)
                {
                    System.Numerics.Complex dataInc = _freqDomainAmpsInc[i][dimIndex0];
                    System.Numerics.Complex data = freqDomainDatas[i][dimIndex0];
